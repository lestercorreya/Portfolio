...
const ResumeSection = (props) => {
  ...
  return (
    <div className='px-5 min-h-regularScreen bg-white flex flex-col relative items-center justify-center py-16 lg:flex-row lg:px-48 md:px-20' ref={(node) => setRefs(node, "resume")}>
      <div className='flex-1 px-16 flex justify-center flex-col mb-16 transition-all duration-500 ease-in delay-300 w-full items-center' style={{ transform: resumeInView ? "translateY(0)" : "translateY(200px)", opacity: resumeInView ? "1" : "0" }}>
      ...
      <div className='flex-1 justify-center items-center flex drop-shadow-xl z-10 relative' style={{ perspective: "100px" }}>
        <Image src={resumeImage} alt="Resume" className='w-full transition-all duration-500 ease-in' style={{ transform: resumeInView ? "translateZ(0)" : "translateZ(-10px)" }} />
        <div className='w-full h-full absolute' style={{ backdropFilter: resumeInView ? "" : "grayscale(90%)" }}></div>
      </div>
    </div >
  )
}
